...
class ProductDetailsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('backend.products.product-details.edit', compact('product_details','categories','fabric_composition','product_fabric','collections','thumbnails','galleryImages'));
    }
    ...
    public function update(Request $request, $id)
{
    ...
    if ($request->hasFile('thumbnail_image')) {
        ...
        if ($product->thumbnail_image) {
            $existingThumbnails = json_decode($product->thumbnail_image, true);
            foreach ($existingThumbnails as $existingThumbnail) {
                $existingPath = public_path('/murupp/product/thumbnails/' . $existingThumbnail);
                if (file_exists($existingPath)) {
                    unlink($existingPath); // Delete old thumbnail image
                }
            }
        }
    ...
    if ($request->hasFile('gallery_image')) {
        ...
        if ($product->gallery_images) {
            ...
            foreach ($existingGalleryImages as $existingGalleryImage) {
                $existingPath = public_path('/murupp/product/gallery/' . $existingGalleryImage);
                if (file_exists($existingPath)) {
                    unlink($existingPath); // Delete old gallery image
                }
            }
        }
